...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        await message.reply(START_MESSAGE.format(user=message.from_user.mention if message.from_user else message.chat.title, bot=temp.B_LINK), disable_web_page_preview=True)                    
...
if len(message.command) != 2:
    buttons = None  # Set buttons to None to remove the buttons
    ...
    await message.reply_photo(
        ...
        reply_markup=buttons,  # Use the buttons variable here
